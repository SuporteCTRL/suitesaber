...
$font_loc=dirname(__FILE__)."/"."FreeMonoBold.ttf";
...
function barcode_outimage($text, $bars, $scale = 1, $mode = "png",
    ...
    if ($scale<1) $scale=2;
    $total_y=(int)($total_y);
    if ($total_y<1) $total_y=(int)$scale * 50; /* defaul: 60 */
    if (!$space)
/* default       $space=array('top'=>2*$scale,'bottom'=>2*$scale,'left'=>2*$scale,'right'=>2*$scale); */
      $space=array('top'=>2*$scale,'bottom'=>10*$scale,'left'=>2*$scale,'right'=>2*$scale);
